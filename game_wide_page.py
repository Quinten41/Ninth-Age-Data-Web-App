...
def game_wide_page(tournament_type, faction_keys, magic_paths, list_data, unit_data, option_data, num_games):
    ...
    if additional_data == 'None':
        ...
        # For some reason this seaborn code seems to not work
        # fig,ax = plt.subplots()
        # sns.histplot(
        #     data=list_data.to_pandas(),
        #     x='Faction', hue='Turn', multiple='stack', shrink=0.8, ax=ax
        # )

        turns = ['First', 'Second']
        counts = {turn: [] for turn in turns}
        for fac in faction_keys:
            for turn in turns:
                count = list_data.filter((pl.col('Faction') == fac) & (pl.col('Turn') == turn)).height
                counts[turn].append(count)
